...
int main()
{
    struct DynamicArray dynamic_array = dynamic_array_construct(sizeof (long), 2 , 1.5);
    long j = 1;
    for(long i = 0; i < 99; i++)
        dynamic_array.push_front(&dynamic_array, &i);
    dynamic_array.insert(&dynamic_array, -1, &j);
